...
class ConexaoBD(IConexaoBD):
    ...
    # quantidade máxima de conexões do pool
    __tamanho_pool = 5
    ...
    def __new__(cls):
        ...
        if cls.__singleton is None:
            ...
            # cria a lista que armazenará as conexões do pool
            cls.__singleton.__pool_conexoes = []

            # controla qual conexão será retornada na próxima chamada
            cls.__singleton.__indice_conexao = 0

            # cria as 5 conexões com o banco de dados
            for numero in range(cls.__tamanho_pool):
                ...
                # adiciona a conexão criada no pool
                cls.__singleton.__pool_conexoes.append(conexao)

                # mensagem para debug
                cprint(
                    f'\n ** Conectou ao Banco de Dados - Conexão nº {numero + 1} ** \n',
                    "white",
                    "on_light_red",
                    attrs=['bold']
                )

        # retorna a instância única da classe ConexaoBD
        return cls.__singleton

    def obter_conexao(self):
        '''Retorna uma conexão do pool usando o padrão Singleton'''

        # pega a conexão atual do pool
        conexao = self.__pool_conexoes[self.__indice_conexao]

        # mensagem para debug
        cprint(
            f'\n ** Retornando conexão nº {self.__indice_conexao + 1} do Pool ** \n',
            "white",
            "on_blue",
            attrs=['bold']
        )

        # atualiza o índice para a próxima conexão
        self.__indice_conexao += 1

        # se chegou ao final do pool, volta para a primeira conexão
        if self.__indice_conexao >= self.__tamanho_pool:
            self.__indice_conexao = 0
